...
inline void normalize_l2(const float* input, float* output, int count,
    ...
    for (; i + 7 < count; i += 8) {
        _mm256_storeu_ps(
            output + i,
            _mm256_mul_ps(_mm256_loadu_ps(input + i), factor));
    }
    for (; i < count; ++i)
...
void process_heads(
    ...
    for (int vh = vh_begin; vh < vh_end; ++vh) {
        ...
        for (int token = 0; token < seq_len; ++token) {
            ...
            int i = 0;
            for (; i + 7 < state_size; i += 8) {
                _mm256_storeu_ps(
                    head_state + i,
                    _mm256_mul_ps(_mm256_loadu_ps(head_state + i),
                                  decay_vector));
            }
            for (; i < state_size; ++i)
                head_state[i] *= decay;
            ...
            for (int dk = 0; dk < k_dim; ++dk) {
                const __m256 key_vector = _mm256_set1_ps(key[dk]);
                ...
                for (; dv + 7 < v_dim; dv += 8) {
                    _mm256_storeu_ps(
                        memory.data() + dv,
                        _mm256_fmadd_ps(
                            _mm256_loadu_ps(state_row + dv), key_vector,
                            _mm256_loadu_ps(memory.data() + dv)));
                }
                for (; dv < v_dim; ++dv)
            ...
            for (; dv + 7 < v_dim; dv += 8) {
                _mm256_storeu_ps(
                    delta.data() + dv,
                    _mm256_mul_ps(
                        _mm256_sub_ps(_mm256_loadu_ps(value.data() + dv),
                                      _mm256_loadu_ps(memory.data() + dv)),
                        beta_vector));
            }
            for (; dv < v_dim; ++dv)
            ...
            for (int dk = 0; dk < k_dim; ++dk) {
                ...
                for (; d + 7 < v_dim; d += 8) {
                    _mm256_storeu_ps(
                        state_row + d,
                        _mm256_fmadd_ps(
                            key_vector, _mm256_loadu_ps(delta.data() + d),
                            _mm256_loadu_ps(state_row + d)));
                }
                for (; d < v_dim; ++d)
            ...
            for (int dk = 0; dk < k_dim; ++dk) {
                const __m256 query_vector = _mm256_set1_ps(query[dk]);
                const float* state_row =
                    head_state + static_cast<size_t>(dk) * v_dim;
                int d = 0;
                for (; d + 7 < v_dim; d += 8) {
                    _mm256_storeu_ps(
                        attention.data() + d,
                        _mm256_fmadd_ps(
                            _mm256_loadu_ps(state_row + d), query_vector,
                            _mm256_loadu_ps(attention.data() + d)));
                }
                for (; d < v_dim; ++d)
            ...
            const float rms = 1.0f /
                std::sqrt(sum_squares(attention.data(), v_dim) *
                          ...
                          rms_epsilon);
            const __m256 norm_factor =
                _mm256_set1_ps(rms * output_scale);
            const float* z_row =
            ...
            for (; d + 7 < v_dim; d += 8) {
                _mm256_storeu_ps(
                    attention.data() + d,
                    _mm256_mul_ps(
                        _mm256_mul_ps(_mm256_loadu_ps(attention.data() + d),
                                      norm_factor),
                        _mm256_loadu_ps(norm_weight + d)));
            }
            for (; d < v_dim; ++d)
